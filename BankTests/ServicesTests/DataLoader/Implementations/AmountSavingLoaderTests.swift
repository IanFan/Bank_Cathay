...
final class AmountSavingLoaderTests: XCTestCase {
    var loader: AmountSavingLoader!
    
    override func setUpWithError() throws {
        super.setUp()
        
        let params = FileParams_amountSaving(fileName: "usdSavings1", fileExt: "json")
        let key = params.cacheKey
        JsonHelper().clearJson(forKey: key)
        
        loader = AmountSavingLoader()
    }
    ...
    func testLoadDataFromCache() async throws {
        ...
        case .success(let responseModel):
            ...
            case .success(let responseModel):
                ...
                XCTAssertTrue(responseModel.result?.savingsList.count ?? 0 > 0)
