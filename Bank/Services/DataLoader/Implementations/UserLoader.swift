...
struct UserLoader: GenericSingleDataLoaderProtocol {
    ...
    func loadDataFromCache(params: Params) throws -> Result<ResultType, Error> {
        ...
        guard let model = try parse(params: DataParseParams_user(data: data)) else {
    ...
    private func loadOnlineFile(params: Params) async throws -> Params? {
        let loader = LoadFileStrategy_user()
    ...
    private func parse(params: DataParseParams) throws -> UserResponseModel? {
        switch params {
        case let params as DataParseParams_user:
            let parser = ParseStrategy_user()
