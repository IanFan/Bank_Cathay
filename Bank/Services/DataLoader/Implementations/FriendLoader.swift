...
struct FriendLoader: GenericSingleDataLoaderProtocol {
    ...
    func loadDataFromCache(params: Params) throws -> Result<ResultType, Error> {
        ...
        guard let model = try parse(params: DataParseParams_friend(data: data)) else {
    ...
    private func loadOnlineFile(params: Params) async throws -> Params? {
        let loader = LoadFileStrategy_friend()
    ...
    private func parse(params: DataParseParams) throws -> FriendResponseModel? {
        switch params {
        case let params as DataParseParams_friend:
            let parser = ParseStrategy_friend()
