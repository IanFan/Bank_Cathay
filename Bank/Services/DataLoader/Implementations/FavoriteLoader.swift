...
struct FavoriteLoader: GenericSingleDataLoaderProtocol {
    ...
    func loadDataFromCache(params: Params) throws -> Result<ResultType, Error> {
        ...
        guard let model = try parse(params: DataParseParams_favorite(data: data)) else {
    ...
    private func loadOnlineFile(params: Params) async throws -> Params? {
        let loader = LoadFileStrategy_favorite()
    ...
    private func parse(params: DataParseParams) throws -> FavoriteResponseModel? {
        switch params {
        case let params as DataParseParams_favorite:
            let parser = ParseStrategy_favorite()
